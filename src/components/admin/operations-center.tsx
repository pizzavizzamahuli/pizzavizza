...
export default function OperationsCenter() {
  ...
  const selectedDeliveryStaff = selected && selected.fulfillmentType === 'DELIVERY' && data?.staff?.length
    ? data.staff.find((staff) => staff.id === (selected.deliveryStaffId || assignmentStaffId)) || null
    : null;
  ...
  return <div className="operations-center space-y-6">
    ...
    {selected ? <div className="fixed inset-0 z-50 bg-stone-950/40" onClick={() => setSelected(null)}><aside className="absolute right-0 top-0 h-full w-full max-w-2xl overflow-y-auto bg-stone-50 p-5 shadow-2xl sm:p-8" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-600">{kind === 'order' ? 'Order operations' : 'Reservation operations'}</p><h2 className="mt-2 text-2xl font-semibold">{selected.orderNumber || selected.bookingNumber}</h2></div><button type="button" onClick={() => setSelected(null)} className="rounded-full border px-3 py-2">Close</button></div>{kind === 'order' && selected.fulfillmentType === 'DELIVERY' ? <><section className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-5">{['READY'].includes(selected.orderStatus) ? <><h3 className="font-semibold text-amber-900">Assign Delivery Staff</h3><p className="mt-1 text-sm text-amber-800">{selected.deliveryStaffName ? 'You can reassign this delivery before pickup.' : 'This READY delivery is waiting for assignment.'}</p>{data?.staff?.length ? <><select value={assignmentStaffId || selected.deliveryStaffId || ''} onChange={(event) => setAssignmentStaffId(event.target.value)} className="mt-3 w-full rounded-xl border border-amber-300 bg-white px-3 py-2"><option value="">Select delivery staff</option>{data.staff.map((staff) => <option key={staff.id} value={staff.id}>{staff.name} ({staff.userCode || 'No user ID'} • {staff.mobile || 'No phone'})</option>)}</select><button type="button" disabled={!assignmentStaffId && !selected.deliveryStaffId} onClick={() => update(`/api/admin/orders/${selected.orderNumber}/assignment`, { staffId: assignmentStaffId || selected.deliveryStaffId })} className="mt-3 rounded-full bg-amber-600 px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">{selected.deliveryStaffId ? 'Save reassignment' : 'Save assignment'}</button></> : <p className="mt-3 text-sm font-medium text-amber-900">No eligible delivery staff is currently available.</p>}</> : <><h3 className="font-semibold text-amber-900">Assigned delivery staff</h3><div className="mt-3 rounded-xl border border-amber-200 bg-white p-3 text-sm text-stone-700"><p className="font-semibold text-stone-900">{selected.deliveryStaffName || 'Delivery staff not assigned'}</p>{selectedDeliveryStaff ? <><p className="mt-1">User ID: {selectedDeliveryStaff.userCode || 'N/A'}</p><p className="mt-1">Phone: {selectedDeliveryStaff.mobile || 'N/A'}</p><p className="mt-1">Status: {selectedDeliveryStaff.accountStatus || 'ACTIVE'}</p></> : <><p className="mt-1">User ID: {selected.deliveryStaffId || 'N/A'}</p><p className="mt-1">Phone: N/A</p></>}</div></>}</section>{selected.deliveryStaffId || selected.deliveryStaffName || selectedDeliveryStaff ? <section className="mt-4 rounded-2xl border border-sky-200 bg-sky-50 p-5"><h3 className="font-semibold text-sky-900">Delivery staff details</h3><div className="mt-3 grid gap-3 sm:grid-cols-2 text-sm text-sky-900"><div className="rounded-xl bg-white p-3"><p className="text-xs uppercase tracking-wide text-sky-700">Name</p><p className="mt-1 font-semibold">{selected.deliveryStaffName || selectedDeliveryStaff?.name || 'Not assigned'}</p></div><div className="rounded-xl bg-white p-3"><p className="text-xs uppercase tracking-wide text-sky-700">User ID</p><p className="mt-1 font-semibold font-mono">{selectedDeliveryStaff?.userCode || selected.deliveryStaffId || 'N/A'}</p></div><div className="rounded-xl bg-white p-3"><p className="text-xs uppercase tracking-wide text-sky-700">Phone</p><p className="mt-1 font-semibold">{selectedDeliveryStaff?.mobile || 'N/A'}</p></div><div className="rounded-xl bg-white p-3"><p className="text-xs uppercase tracking-wide text-sky-700">Status</p><p className="mt-1 font-semibold">{selectedDeliveryStaff?.accountStatus || (selected.orderStatus === 'DELIVERED' ? 'Delivered' : 'Assigned')}</p></div></div></section> : null}</> : null}{kind === 'order' && selected.fulfillmentType === 'DELIVERY' ? <section className="mt-4 rounded-2xl border border-sky-200 bg-sky-50 p-4"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-sky-700">Assigned delivery staff</p><p className="mt-2 text-sm font-semibold text-sky-900">{selected.deliveryStaffName || (selected.deliveryStaffId ? `Staff ID: ${selected.deliveryStaffId}` : 'Unassigned')}</p>{selected.deliveryStaffId ? <p className="mt-1 text-xs text-sky-700">Order assigned to this staff member</p> : <p className="mt-1 text-xs text-sky-700">No delivery staff assigned yet</p>}</section> : null}<OperationDetail data={data} selected={selected} kind={kind} update={update} /></aside></div> : null}
  </div>;
}
