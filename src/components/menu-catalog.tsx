...
function documentId(document: { _id?: unknown; id?: string; slug?: string }) {
  const value = document._id as { toHexString?: () => string } | string | undefined;
  if (typeof value === 'string' && value) return value;
  if (value && typeof value === 'object' && typeof value.toHexString === 'function') return value.toHexString();
  return document.id || document.slug || '';
}
...
export default function MenuCatalog({ categories, products, bookingNumber }: { categories: CategoryDocument[]; products: ProductDocument[]; bookingNumber: string | null }) {
  ...
  async function changeQuantity(product: ProductDocument, quantity: number) {
    const productId = documentId(product);
