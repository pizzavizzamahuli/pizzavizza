...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!(file instanceof File) || !file.type.startsWith('image/')) {
      return NextResponse.json({ error: 'Please select a menu image file' }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Menu upload failed' }, { status: 500 });
  }
}
