...
export default function RestaurantSettingsForm({ isMainAdmin = false }: { isMainAdmin?: boolean }) {
  ...
  return (
    <div className="min-w-0 space-y-4">
      ...
      <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-sm font-semibold text-stone-900">Special dates</h2><p className="mt-1 text-xs text-stone-500">Set holidays, one-day closures, or special opening hours.</p></div><button type="button" onClick={() => setSettings({ ...settings, specialDates: [...(settings.specialDates || []), { date: '', isOpen: false, openTime: null, closeTime: null, label: '' }] })} className="rounded-full bg-stone-900 px-3 py-2 text-xs font-semibold text-white">Add special date</button></div><div className="mt-4 space-y-2">{(settings.specialDates || []).map((item: { date: string; isOpen: boolean; openTime?: string | null; closeTime?: string | null; label?: string | null }, index: number) => <div key={`${item.date}-${index}`} className="grid gap-2 rounded-xl border border-stone-200 bg-white p-3 sm:grid-cols-[minmax(8rem,1fr)_auto_minmax(7rem,1fr)_minmax(7rem,1fr)_auto]"><input type="date" className="input" value={item.date} onChange={(event) => setSettings({ ...settings, specialDates: settings.specialDates.map((entry: typeof item, entryIndex: number) => entryIndex === index ? { ...entry, date: event.target.value } : entry) })} /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={item.isOpen} onChange={(event) => setSettings({ ...settings, specialDates: settings.specialDates.map((entry: typeof item, entryIndex: number) => entryIndex === index ? { ...entry, isOpen: event.target.checked } : entry) })} /> Open</label><input type="time" disabled={!item.isOpen} className="input" value={item.openTime || ''} onChange={(event) => setSettings({ ...settings, specialDates: settings.specialDates.map((entry: typeof item, entryIndex: number) => entryIndex === index ? { ...entry, openTime: event.target.value } : entry) })} /><input type="time" disabled={!item.isOpen} className="input" value={item.closeTime || ''} onChange={(event) => setSettings({ ...settings, specialDates: settings.specialDates.map((entry: typeof item, entryIndex: number) => entryIndex === index ? { ...entry, closeTime: event.target.value } : entry) })} /><button type="button" onClick={() => setSettings({ ...settings, specialDates: settings.specialDates.filter((_: typeof item, entryIndex: number) => entryIndex !== index) })} className="rounded-full border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-700">Remove</button><input className="input sm:col-span-5" placeholder="Label, e.g. Independence Day" value={item.label || ''} onChange={(event) => setSettings({ ...settings, specialDates: settings.specialDates.map((entry: typeof item, entryIndex: number) => entryIndex === index ? { ...entry, label: event.target.value } : entry) })} /></div>)}</div></div>
      ...
      <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-sm font-semibold text-stone-900">Restaurant Availability</h2><p className="mt-1 text-xs text-stone-500">Weekly hours use the restaurant timezone and are enforced by the backend.</p></div><select className="input w-auto min-w-40" value={settings.manualAvailabilityOverride || ''} onChange={(event) => setSettings({ ...settings, manualAvailabilityOverride: event.target.value || null })}><option value="">Follow schedule</option><option value="OPEN">Open manually</option><option value="CLOSED">Close manually</option></select></div>
        {settings.manualAvailabilityOverride === 'CLOSED' ? <input className="input mt-3 w-full" maxLength={200} value={settings.manualAvailabilityReason || ''} onChange={(event) => setSettings({ ...settings, manualAvailabilityReason: event.target.value })} placeholder="Closure reason, e.g. Kitchen maintenance" /> : null}
        <label className="mt-3 block max-w-xs text-sm"><span className="mb-1 block">Restaurant timezone</span><input className="input w-full" value={settings.restaurantTimezone || 'Asia/Kolkata'} onChange={(event) => setSettings({ ...settings, restaurantTimezone: event.target.value })} /></label>
        <div className="mt-4 space-y-2">{scheduleDays.map(([key, label]) => { const day = settings.weeklySchedule?.[key] || { isOpen: true, openTime: '11:00', closeTime: '23:00' }; return <div key={key} className="grid min-w-0 gap-2 rounded-xl border border-stone-200 bg-white p-3 sm:grid-cols-[minmax(7rem,1fr)_auto_minmax(7rem,1fr)_minmax(7rem,1fr)] sm:items-center"><span className="font-medium text-stone-800">{label}</span><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={day.isOpen} onChange={(event) => setSettings({ ...settings, weeklySchedule: { ...settings.weeklySchedule, [key]: { ...day, isOpen: event.target.checked } } })} /> Open</label><label className="text-sm"><span className="sr-only">{label} opens</span><input type="time" disabled={!day.isOpen} className="input w-full" value={day.openTime} onChange={(event) => setSettings({ ...settings, weeklySchedule: { ...settings.weeklySchedule, [key]: { ...day, openTime: event.target.value } } })} /></label><label className="text-sm"><span className="sr-only">{label} closes</span><input type="time" disabled={!day.isOpen} className="input w-full" value={day.closeTime} onChange={(event) => setSettings({ ...settings, weeklySchedule: { ...settings.weeklySchedule, [key]: { ...day, closeTime: event.target.value } } })} /></label></div>; })}</div>
      </div>
