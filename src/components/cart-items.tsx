...
export default function CartItems({ items, onItemsChange }: { items: CartItem[]; onItemsChange?: (items: CartItem[]) => void }) {
  ...
  return <ul className="mt-6 space-y-4">{cartItems.map((item) => <li key={item.itemKey || item.productId} className="overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-sm"><div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-start"><div className="flex min-w-0 flex-1 items-start gap-3 sm:gap-4">{item.image ? <img src={item.image} alt={item.name || 'Cart item'} className="h-16 w-16 flex-shrink-0 rounded-2xl object-cover sm:h-20 sm:w-20" /> : <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-2xl bg-stone-100 text-xs text-stone-500 sm:h-20 sm:w-20">No image</div>}<div className="min-w-0 flex-1"><div className="break-words text-base font-semibold leading-6 text-stone-900 sm:text-lg">{item.name || 'Item'}</div>{item.selectedSize ? <div className="mt-1 text-sm text-stone-600">Size: {item.selectedSize.name}</div> : null}{item.selectedOptions?.length ? <div className="mt-2 space-y-1 break-words text-xs leading-5 text-stone-600 sm:text-sm">{item.selectedOptions.map((option) => <div key={option.optionId}><span className="font-medium">{option.groupName}</span>: {option.optionName}{option.quantity && option.quantity > 1 ? ` × ${option.quantity}` : ''}</div>)}</div> : null}<div className="mt-3"><QuantityControl quantity={item.quantity} onChange={(next) => update(item, next)} /></div></div></div><div className="flex shrink-0 items-center justify-between gap-4 border-t border-stone-100 pt-3 sm:min-w-32 sm:flex-col sm:items-end sm:border-t-0 sm:pt-1"><div className="text-base font-semibold text-stone-900">₹{(item.unitPrice || 0).toFixed(2)}</div><div className="text-sm text-stone-500">Total ₹{((item.unitPrice || 0) * item.quantity).toFixed(2)}</div></div></div></li>)}</ul>;
}