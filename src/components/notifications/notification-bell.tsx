  ...
  return (
    <div className="relative z-[70] shrink-0">
      ...
      {open ? <div className={popupClasses}>
        <div className={`flex items-center justify-between px-4 py-3 ${getPopupInnerClasses(popupStyle)}`}><div><h2 className={`font-semibold ${textClasses}`}>Notifications</h2><p className={`text-xs ${mutedTextClasses}`}>{unreadCount} unread</p></div><button type="button" onClick={() => void markAllRead()} className={`text-xs font-semibold ${actionTextClasses}`}>Mark all read</button></div>
