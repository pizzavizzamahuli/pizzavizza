...
export default function CartItems({ items, onItemsChange }: { items: CartItem[]; onItemsChange?: (items: CartItem[]) => void }) {
  ...
  return <ul className="mt-6 space-y-4">{cartItems.map((item) => <li key={item.itemKey || item.productId} className="overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-sm"><div className="grid gap-4 p-4 sm:grid-cols-[1fr_auto] sm:items-center"><div className="flex min-w-0 items-start gap-4">{item.image ? <img src={item.image} alt={item.name || 'Cart item'} className="h-20 w-20 flex-shrink-0 rounded-2xl object-cover" /> : <div className="flex h-20 w-20 flex-shrink-0 items-center justify-center rounded-2xl bg-stone-100 text-sm text-stone-500">No image</div>}<div className="min-w-0"><div className="break-words text-lg font-semibold text-stone-900">{item.name || 'Item'}</div><div className="mt-3"><QuantityControl quantity={item.quantity} onChange={(next) => update(item, next)} /></div>{item.selectedOptions?.length ? <div className="mt-3 space-y-1 break-words text-sm text-stone-600">{item.selectedOptions.map((option) => <div key={option.optionId}><span className="font-medium">{option.groupName}</span>: {option.optionName}</div>)}</div> : null}</div></div><div className="min-w-[8rem] whitespace-nowrap text-right text-sm sm:text-base"><div className="text-base font-semibold text-stone-900">₹{(item.unitPrice || 0).toFixed(2)}</div><div className="text-sm text-stone-500">Total ₹{((item.unitPrice || 0) * item.quantity).toFixed(2)}</div></div></div></li>)}</ul>;
}