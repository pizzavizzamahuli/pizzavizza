...
export default function LocationMapClient({
  ...
  if (!icons) return <div className="rounded-md border border-stone-200 bg-stone-50" style={{ height }} />;
  ...
  const polyline = routeCoordinates && routeCoordinates.length > 1 ? routeCoordinates.map((point) => [point.latitude, point.longitude] as [number, number]) : [];
  ...
  return (
    <div className="overflow-hidden rounded-md border border-stone-200 bg-stone-50" style={{ height }}>
      <MapContainer center={mapCenter} zoom={13} scrollWheelZoom style={{ height: '100%', width: '100%' }}>
        ...
        />
        {storeLocation && Number.isFinite(storeLocation.latitude) && Number.isFinite(storeLocation.longitude) ? <Marker position={[storeLocation.latitude, storeLocation.longitude]} icon={icons.store} /> : null}
        {customerLocation && Number.isFinite(customerLocation.latitude) && Number.isFinite(customerLocation.longitude) ? <Marker position={[customerLocation.latitude, customerLocation.longitude]} icon={icons.marker} /> : null}
