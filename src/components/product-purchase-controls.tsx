...
export default function ProductPurchaseControls({ productId, bookingNumber }: { productId: string; bookingNumber?: string | null }) {
  ...
  return <div className="flex flex-wrap items-center gap-3"><QuantityControl quantity={quantity} onChange={async (next) => setQuantity(Math.max(1, next))} /><AddToCartButton productId={productId} bookingNumber={bookingNumber} quantity={quantity} /><BuyNowButton productId={productId} bookingNumber={bookingNumber} quantity={quantity} /></div>;
}
