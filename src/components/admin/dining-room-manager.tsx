...
import DiningRoomForm, { type AdminDiningRoom } from '@/src/components/admin/dining-room-form';

export default function DiningRoomManager({ initialRooms }: { initialRooms: AdminDiningRoom[] }) {
  ...
  function handleSaved(room: AdminDiningRoom) {
    setRooms((current) => {
      ...
      return exists ? current.map((item) => item._id === room._id ? room : item) : [room, ...current];
    });
    setEditingRoom(null);
  }

  async function archiveRoom(room: AdminDiningRoom) {
    if (!window.confirm(`Deactivate ${room.name}? Existing booking history will be preserved.`)) return;
    const response = await fetch(`/api/admin/dining/rooms/${room._id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isActive: false, isBookable: false }) });
