  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-stone-200 bg-white p-6">
        ...
        <button onClick={saveWalletAdjustment} disabled={saving} className="mt-4 rounded bg-amber-600 px-4 py-2 text-white">{saving ? 'Saving…' : 'Adjust wallet'}</button>
          ...
          {wallets.length === 0 ? <p className="text-sm text-stone-600">No wallet balances yet.</p> : wallets.map((wallet) => (
            <div key={String(wallet._id || wallet.userId)} className="rounded border border-stone-200 p-3">
              <div className="flex justify-between gap-3">
                <div>
                  <div className="font-medium">{String(wallet.userId)}</div>
